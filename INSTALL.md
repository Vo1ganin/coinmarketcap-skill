...
```bash
tmp="$(mktemp -d)"
git clone --depth 1 https://github.com/Vo1ganin/coinmarketcap-skill.git "$tmp/repo"
rm -rf "$HOME/.claude/skills/coinmarketcap"
mkdir -p "$HOME/.claude/skills"
cp -R "$tmp/repo" "$HOME/.claude/skills/coinmarketcap"
rm -rf "$HOME/.claude/skills/coinmarketcap/.git" "$tmp"
```
